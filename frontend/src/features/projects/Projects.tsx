    ...
    return (
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 animate-pulse">
          {[1, 2, 3].map((i) => (
            <div key={i} className="h-32 bg-zinc-100 rounded-xl border border-zinc-200/50"></div>
          ))}
        </div>
      </div>
    );
        ...
        <div className="flex flex-col items-center justify-center border border-dashed border-zinc-200 rounded-xl bg-white p-12 text-center">
          ...
          {isAdmin ? (
            <>
              ...
              <button
                onClick={() => setIsModalOpen(true)}
                className="mt-4 px-3 py-1.5 border border-zinc-200 hover:bg-zinc-50 text-xs font-semibold rounded-lg transition-colors"
              >
                Create Project
              </button>
            </>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {projects.map((project) => (
            <Link
              ...
              className="group border border-zinc-200 hover:border-black bg-white rounded-xl p-6 flex flex-col justify-between hover:shadow-sm transition-all duration-200"
            >
              <div className="space-y-3">
                <div className="flex items-center gap-2.5">
                  <Folder className="h-4 w-4 text-zinc-400 group-hover:text-black shrink-0" />
                  <h3 className="font-semibold text-sm text-black truncate">{project.name}</h3>
                </div>
      ...
      {isModalOpen && (
        <div className="fixed inset-0 bg-black/40 backdrop-blur-none z-50 flex items-center justify-center p-4">
          <div className="bg-white border border-zinc-200 rounded-xl max-w-md w-full p-6 shadow-lg relative animate-in fade-in zoom-in-95 duration-150">
            ...
            <h3 className="font-bold text-base text-black mb-1">Create Project</h3>
